...
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		...
		var _v2 = A2(
			...
			url,
			{modalVisibility: $rundis$elm_bootstrap$Bootstrap$Modal$hidden, navKey: key, navState: navState, numberOfSeconds: 0, page: $author$project$Main$Home, timerStarted: false, times: _List_Nil});
		var model = _v2.a;
		var urlCmd = _v2.b;
		return _Utils_Tuple2(
			model,
		...
		switch (msg.$) {
			...
			case 'NavMsg':
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'Tick':
				var nos = model.numberOfSeconds;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{numberOfSeconds: nos + 1}),
					$elm$core$Platform$Cmd$none);
			case 'StartTimer':
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'StopTimer':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							timerStarted: false,
							times: A2($elm$core$List$cons, nos, oldTimes)
						}),
					$elm$core$Platform$Cmd$none);
			case 'LapTimer':
				var oldTimes = model.times;
			...
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{numberOfSeconds: 0, times: _List_Nil}),
					$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Main$pageAbout = function (_v0) {
	return _List_fromArray(
		[
			A2(
			$elm$html$Html$h1,
			...
			$elm$html$Html$text('TODO: About VG soft and link to repo')
		]);
};
...
var $author$project$Main$LapTimer = {$: 'LapTimer'};
var $author$project$Main$StartTimer = {$: 'StartTimer'};
var $author$project$Main$messageFromButton = function (model) {
	var _v0 = model.timerStarted;
	if (_v0) {
		return $author$project$Main$LapTimer;
	} else {
		return $author$project$Main$StartTimer;
	}
};
...
var $author$project$Main$secondsToTimeForDisplay = function (numberOfSeconds) {
	var seconds = A2($elm$core$Basics$modBy, 60, numberOfSeconds);
	var hours = (numberOfSeconds / 3600) | 0;
	var minutes = ((numberOfSeconds - (hours * 3600)) / 60) | 0;
...
var $author$project$Main$showResetButtonIfNeeded = function (model) {
	return ((!model.timerStarted) && (model.numberOfSeconds > 0)) ? A2(
...
var $author$project$Main$StopTimer = {$: 'StopTimer'};
var $author$project$Main$showStopButtonIfNeeded = function (model) {
	var _v0 = model.timerStarted;
	if (_v0) {
		return A2(
			$rundis$elm_bootstrap$Bootstrap$Button$button,
...
var $author$project$Main$convertToTimeFormat = function (number) {
	return (number < 10) ? $elm$core$String$concat(
		_List_fromArray(
			[
				'0',
...
var $author$project$Main$timeForDisplayToString = function (timeForDisplay) {
	return A2(
		$elm$core$String$join,
		':',
		_List_fromArray(
			[
				$author$project$Main$convertToTimeFormat(timeForDisplay.hours),
				$author$project$Main$convertToTimeFormat(timeForDisplay.minutes),
				$author$project$Main$convertToTimeFormat(timeForDisplay.seconds)
			]));
};
var $author$project$Main$titleForButton = function (model) {
	var _v0 = model.timerStarted;
	if (_v0) {
		return 'Lap';
	} else {
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$author$project$Main$timeForDisplayToString(
										$author$project$Main$secondsToTimeForDisplay(model.numberOfSeconds)))
								])),
							A2(
							$rundis$elm_bootstrap$Bootstrap$Button$button,
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											...
											$author$project$Main$messageFromButton(model))
										]))
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(
									$author$project$Main$titleForButton(model))
								])),
							$author$project$Main$showStopButtonIfNeeded(model),
							$author$project$Main$showResetButtonIfNeeded(model)
						]))
				])),
			A2(
			$rundis$elm_bootstrap$Bootstrap$Grid$row,
									...
									return A2(
										...
										_List_fromArray(
											[
												$elm$html$Html$text(
												$author$project$Main$timeForDisplayToString(
													$author$project$Main$secondsToTimeForDisplay(time)))
											]));
								},
								model.times))
						]))
	...
	return A2(
		...
		A2(
			...
			A3(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Elm Stopwatch')
					]),
